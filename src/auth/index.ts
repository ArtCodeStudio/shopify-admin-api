...
function buildHashString(
  ...
  const kvps = Object.getOwnPropertyNames(querystring)
    ...
    .map(
      (key) =>
        `${replaceChars(key, true)}=${replaceChars(querystring[key], false)}`,
    )
    .join(type === 'web' ? '&' : '');
...
function getHmacHash(
  ...
  let hash: string | crypto.lib.WordArray = crypto.HmacSHA256(
    hashString,
    secretKey,
  );
 ...
 */
export async function isAuthenticRequest(
  querystring: { [index: string]: any },
  ...
  const computed = getHmacHash(
    shopifySecretKey,
    buildHashString('web', querystring),
  );
 ...
 * @returns a boolean indicating whether the request is authentic or not.
 */
export async function isAuthenticProxyRequest(
  querystring: { [index: string]: any },
  shopifySecretKey: string,
) {
  ...
  const computed = getHmacHash(
    shopifySecretKey,
    buildHashString('proxy', querystring),
  );
...
export async function isAuthenticWebhook(
  headers: { [index: string]: any } | string,
  requestBody: string,
  shopifySecretKey: string,
) {
  let hmac: string;
...
export async function authorize(
  ...
  const response = await new AuthorizeService(shopDomain).authorize(
    shopifyApiKey,
    shopifySecretKey,
    code,
  );
